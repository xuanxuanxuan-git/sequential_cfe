...
class HELOC(gym.Env):
    ...
    def __init__(self, dist_lambda):
        super(HELOC, self).__init__()
        file1 = f"{os.path.dirname(os.path.realpath(__file__))}/heloc.csv"
        clf, dataset, scaler, X_test, X_train = classifier.train_model_heloc(file=file1, parameter=1)
        ...
        self.test_dataset = scaler.transform(X_test)
        self.state_count = 0
        self.scaler = scaler
        self.classifier = clf
        self.states = {}
        ...
        try:
            self.undesirable_x = np.load(f"{os.path.dirname(os.path.realpath(__file__))}/../../../datapoints_to_generate_cfes/undesirable_x_heloc.npy")
            print("Found")
        except:
            undesirable_x = []
            ...
            np.save(f"{os.path.dirname(os.path.realpath(__file__))}/../../../datapoints_to_generate_cfes/undesirable_x_heloc.npy", undesirable_x)

        print(len(self.undesirable_x), "Total datapoints to run the approach on")
        self.action_seq = []
        self.kde = stats.gaussian_kde(self.test_dataset.T)
        self.reset()
...
class HELOC0(HELOC):
    def __init__(self, enable_render=True):
        super(HELOC0, self).__init__(dist_lambda=0.0)


class HELOC01(HELOC):
    def __init__(self, enable_render=True):
        super(HELOC01, self).__init__(dist_lambda=0.1)
