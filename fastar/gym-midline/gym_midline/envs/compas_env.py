...
class Compas(gym.Env):
    ...
    def __init__(self, dist_lambda):
        ...
        file1 = f"{os.path.dirname(os.path.realpath(__file__))}/compas.csv"
        clf, dataset, scaler, X_test, X_train = classifier.train_model_compas(file=file1, parameter=1)
        ...
        self.immutable_features = ['race_Other', 'sex_Male']
        ...
        self.undesirable_x = []
        try:
            self.undesirable_x = np.load(f"{os.path.dirname(os.path.realpath(__file__))}/../../../datapoints_to_generate_cfes/undesirable_x_compas.npy")
            print("Found")
        except:
            undesirable_x = []
...
class Compas0(Compas):
    def __init__(self, enable_render=True):
        super(Compas0, self).__init__(dist_lambda=0.0)


class Compas01(Compas):
    def __init__(self, enable_render=True):
        super(Compas01, self).__init__(dist_lambda=0.1)
